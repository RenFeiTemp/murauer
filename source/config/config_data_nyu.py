...
args_data.dataset_type = basetypes.DatasetType.NYU

# Change to point to the original NYU dataset
args_data.nyu_data_basepath = "/data/users/pfren/data/dataset/hand/nyu"

# If a "cache" should be used (=> faster loading/training), change the path
args_data.use_pickled_cache = False
args_data.nyu_data_basepath_pickled = "../cache"
...
args_data.cam_ids_for_pose_train_synth = np.asarray([], dtype=np.int32)
args_data.output_cam_ids_train = np.asarray([1])    # for view prediction
args_data.output_cam_ids_test = np.asarray([0])     # for view prediction
...
# Cube-size
args_data.do_jitter_cubesize = [True, True, True]
